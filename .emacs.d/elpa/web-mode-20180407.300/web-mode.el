...
;; Copyright 2011-2018 Fran巽ois-Xavier Bois

;; Version: 16.0.4
;; Package-Version: 20180407.300
;; Author: Fran巽ois-Xavier Bois <fxbois AT Google Mail Service>
;; Maintainer: Fran巽ois-Xavier Bois
...
;; NOTE: il est important d'identifier des caract竪res en fin de ligne
...
;; todo : parsing plus compliqu辿: {$obj->values[3]->name}
...
(defun web-mode-buffer-indent ()
  ...
  (let ((debug t) (ts (current-time)) (sub nil))
    ...
    (when debug
      ...
      (message "buffer-indet: time elapsed = %Ss %9S袖s" (nth 1 sub) (nth 2 sub)))
...
;; todo : passer de r竪gle en r竪gle et mettre un \n à la fin
